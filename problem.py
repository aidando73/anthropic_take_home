...
def reference_kernel2(t: Tree, inp: Input):
    """
    Reference implementation of the kernel.
    ...
    for h in range(inp.rounds):
        ...
        for i in range(len(inp.indices)):
            ...
            # Go back to the top if we've hit the end of the tree
            idx = 0 if idx >= len(t.values) else idx

            # We record the current val - record the next idx
            inp.values[i] = val
            inp.indices[i] = idx
